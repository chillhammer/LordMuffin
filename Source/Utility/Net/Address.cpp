...
namespace Skel::Net
{
	...
	const sockaddr_in Address::GetSockAddr4() const
	{
		ASSERT(GetType() == ADDRESS_IPV4, "Must be IPV4 address");
		sockaddr_in addr;
		memset(&addr, 0, sizeof(addr));
		addr.sin_family = AF_INET;
		addr.sin_port = htons(GetPort());
		addr.sin_addr.s_addr = GetAddress4();
		return addr;
	}

	const sockaddr_in6 Address::GetSockAddr6() const 
	{
		ASSERT(GetType() == ADDRESS_IPV6, "Must be IPV6 address");
		sockaddr_in6 addr;
		memset(&addr, 0, sizeof(addr));
		addr.sin6_family = AF_INET6;
		addr.sin6_port = htons(GetPort());
		memcpy(&addr.sin6_addr, GetAddress6(), sizeof(addr.sin6_addr));
		return addr;
	}
