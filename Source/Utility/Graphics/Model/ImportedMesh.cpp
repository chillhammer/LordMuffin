...
namespace Skel
{
			...
			Mesh()
	{
		...
		SetData(&m_Vertices[0], m_Vertices.size() * sizeof(Vertex), &m_Indicies[0], static_cast<unsigned int>(m_Indicies.size()), texture, m_Color);
	}
	...
	{
		SetBufferLayout(GetBufferLayout());
		SetData(&m_Vertices[0], m_Vertices.size() * sizeof(Vertex), &m_Indicies[0], static_cast<unsigned int>(m_Indicies.size()), m_Texture, color);
	}
	...
	{
		SetBufferLayout(GetBufferLayout());
		SetData((void*)&m_Vertices[0], m_Vertices.size() * sizeof(Vertex), &m_Indicies[0], static_cast<unsigned int>(m_Indicies.size()), other.m_Texture, other.m_Color);
	}
